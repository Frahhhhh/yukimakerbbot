...
def newbot(infos):
    ...
    if uid not in auth:
        say("Per potermi utilizzare devi scrivere un messaggio a @kfrahh "
            "includendo come messaggio il tuo ID, se non sai il tuo ID esegui il comando /myid.\n\n"
            "In questo modo, posso aggiungerti alla lista per crearne uno!.")
        return
    ...
    if Manager.get_bot_count() >= max_bots and "!!" not in infos.text:
        say("Tutti i posti sono stati occupati.\nScrivi a @KFrahh per sapere quando ce ne saranno altri ~")
        return
